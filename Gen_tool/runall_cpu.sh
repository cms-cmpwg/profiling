...
if [ "X$RUNALLSTEPS" != "X" ]; then
  if [ -f step1_igprof.py ]; then
    ...
    timeout $TIMEOUT igprof -pp -z -d -t cmsRun -o ./igprofCPU_step1.gz -- cmsRun step1_igprof.py -j step1_igprof_cpu_JobReport.xml >& step1_igprof_cpu.log
  ...
  if [ -f step2_igprof.py ]; then
    ...
    timeout $TIMEOUT igprof -pp -z -d -t cmsRun -o ./igprofCPU_step2.gz -- cmsRun step2_igprof.py -j step2_igprof_cpu_JobReport.xml >& step2_igprof_cpu.log
...
if [ -f step3_igprof.py ]; then
  ...
  timeout $TIMEOUT igprof -pp -z -d -t cmsRun -o ./igprofCPU_step3.gz -- cmsRun step3_igprof.py -j step3_igprof_cpu_JobReport.xml >& step3_igprof_cpu.log
...
if [ -f step4_igprof.py ]; then
  ...
  timeout $TIMEOUT igprof -pp -d -z -t cmsRun -o ./igprofCPU_step4.gz -- cmsRun step4_igprof.py -j step4_igprof_cpu_JobReport.xml >& step4_igprof_cpu.log
...
if [ -f step5_igprof.py ]; then
  ...
  timeout $TIMEOUT igprof -d -pp -z -t cmsRun -o ./igprofCPU_step5.gz -- cmsRun step5_igprof.py -j step5_igprof_cpu_JobReport.xml >& step5_igprof_cpu.log
