...
if ( echo $outname | grep "reHLT" ); then
  for ((step=0;step<${#steps[@]}; ++step));do
      echo "${steps[$step]} --customise=Validation/Performance/TimeMemoryInfo.py --customise_commands \"process.add_(cms.Service("NVProfilerService", highlightModules = cms.untracked.vstring("siPixelClustersPreSplittingCUDA")))\" --python_filename=step$((step+2))_timememoryinfo.py" >>cmd_ts.sh
      echo "${steps[$step]} --customise Validation/Performance/IgProfInfo.customise  --customise_commands \"process.FEVTDEBUGHLToutput = cms.OutputModule('AsciiOutputModule',outputCommands = process.FEVTDEBUGHLTEventContent.outputCommands);process.RECOSIMoutput = cms.OutputModule('AsciiOutputModule',outputCommands = process.RECOSIMEventContent.outputCommands);process.AODSIMoutput = cms.OutputModule('AsciiOutputModule',outputCommands = process.AODSIMEventContent.outputCommands);process.MINIAODSIMoutput = cms.OutputModule('AsciiOutputModule',outputCommands = process.MINIAODSIMEventContent.outputCommands);process.DQMoutput = cms.OutputModule('AsciiOutputModule',outputCommands = process.DQMEventContent.outputCommands);process.options.numberOfThreads = 1;process.maxEvents.input = 20;process.add_(cms.Service("NVProfilerService", highlightModules = cms.untracked.vstring("siPixelClustersPreSplittingCUDA")))\" --python_filename=step"$((step+2))"_igprof.py" >>cmd_ig.sh
  done
else
  for ((step=0;step<${#steps[@]}; ++step));do
      echo "${steps[$step]} --customise=Validation/Performance/TimeMemoryInfo.py --customise_commands \"process.add_(cms.Service("NVProfilerService", highlightModules = cms.untracked.vstring("siPixelClustersPreSplittingCUDA")))\" --python_filename=step$((step+1))_timememoryinfo.py" >>cmd_ts.sh
      echo "${steps[$step]} --customise Validation/Performance/IgProfInfo.customise  --customise_commands \"process.FEVTDEBUGoutput = cms.OutputModule('AsciiOutputModule',outputCommands = process.FEVTDEBUGEventContent.outputCommands);process.FEVTDEBUGHLToutput = cms.OutputModule('AsciiOutputModule',outputCommands = process.FEVTDEBUGHLTEventContent.outputCommands);process.RECOSIMoutput = cms.OutputModule('AsciiOutputModule',outputCommands = process.RECOSIMEventContent.outputCommands);process.AODSIMoutput = cms.OutputModule('AsciiOutputModule',outputCommands = process.AODSIMEventContent.outputCommands);process.MINIAODSIMoutput = cms.OutputModule('AsciiOutputModule',outputCommands = process.MINIAODSIMEventContent.outputCommands);process.DQMoutput = cms.OutputModule('AsciiOutputModule',outputCommands = process.DQMEventContent.outputCommands);process.options.numberOfThreads = 1;process.maxEvents.input = 20;process.add_(cms.Service("NVProfilerService", highlightModules = cms.untracked.vstring("siPixelClustersPreSplittingCUDA")))\\" --python_filename=step"$((step+1))"_igprof.py"  >>cmd_ig.sh
  done
fi

# For reHLT workflows the steps are shifted
if ( echo $outname | grep -q 'reHLT') ; then
  echo "${steps[0]} --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step2_L1REPACK_HLT.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;process.add_(cms.Service("NVProfilerService", highlightModules = cms.untracked.vstring("siPixelClustersPreSplittingCUDA")))\\" --python_filename=step2_fasttimer.py" >>cmd_ft.sh
  echo "${steps[1]} --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step3_RAW2DIGI_L1Reco_RECO_SKIM_PAT_ALCA_DQM.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;process.add_(cms.Service("NVProfilerService", highlightModules = cms.untracked.vstring("siPixelClustersPreSplittingCUDA")))\\" --python_filename=step3_fasttimer.py" >>cmd_ft.sh
  echo "${steps[2]} --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step4_HARVESTING.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;process.add_(cms.Service("NVProfilerService", highlightModules = cms.untracked.vstring("siPixelClustersPreSplittingCUDA")))\\" --python_filename=step4_fasttimer.py" >>cmd_ft.sh
else
  echo "${steps[0]} --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step1_GEN_SIM.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;process.add_(cms.Service("NVProfilerService", highlightModules = cms.untracked.vstring("siPixelClustersPreSplittingCUDA")))\\" --python_filename=step1_fasttimer.py " >>cmd_ft.sh
  echo "${steps[1]} --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step2_DIGI_L1TrackTrigger_L1_DIGI2RAW_HLT_PU.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;process.add_(cms.Service("NVProfilerService", highlightModules = cms.untracked.vstring("siPixelClustersPreSplittingCUDA")))\\" --python_filename=step2_fasttimer.py" >>cmd_ft.sh
  echo "${steps[2]} --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step3_RAW2DIGI_L1Reco_RECO_RECOSIM_PU.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;process.add_(cms.Service("NVProfilerService", highlightModules = cms.untracked.vstring("siPixelClustersPreSplittingCUDA")))\\" --python_filename=step3_fasttimer.py" >>cmd_ft.sh
  echo "${steps[3]} --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step4_PAT_PU.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;process.add_(cms.Service("NVProfilerService", highlightModules = cms.untracked.vstring("siPixelClustersPreSplittingCUDA")))\\" --python_filename=step4_fasttimer.py" >>cmd_ft.sh
# check for 5th step
if ( echo ${!steps[@]} | grep -q 4 );then
  echo "${steps[4]} --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step5_NANO_PU.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;process.add_(cms.Service("NVProfilerService", highlightModules = cms.untracked.vstring("siPixelClustersPreSplittingCUDA")))\\" --python_filename=step5_fasttimer.py" >>cmd_ft.sh
  fi
fi
. cmd_ft.sh
. cmd_ig.sh
