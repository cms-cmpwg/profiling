...
if ( echo $outname | grep -q 'reHLT') ; then
  ...
  echo "${steps[1]} --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step2_gpu_DIGI_L1TrackTrigger_L1_DIGI2RAW_HLT_PU.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1\" --python_filename=step3_gpu_fasttimer.py --suffix \"-j step3_gpu_fasttimer_JobReport.xml\"" >>cmd_ft.sh
