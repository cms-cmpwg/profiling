...
# For reHLT workflows the steps are shifted
if ( echo $outname | grep -q '136') ; then
  echo "${steps[0]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step2_gpu.resources.json');process.FastTimerService.enableDQMbyLumiSection = cms.untracked.bool(False);process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step2_gpu_fasttimer.py" >>cmd_ft.sh
  echo "${steps[1]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step3_gpu.resources.json');process.FastTimerService.enableDQMbyLumiSection = cms.untracked.bool(False);process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step3_gpu_fasttimer.py" >>cmd_ft.sh
  echo "${steps[2]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step4_gpu.resources.json');process.FastTimerService.enableDQMbyLumiSection = cms.untracked.bool(False);process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step4_gpu_fasttimer.py" >>cmd_ft.sh
else
  echo "${steps[0]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step1_gpu.resources.json');process.FastTimerService.enableDQMbyLumiSection = cms.untracked.bool(False);process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step1_gpu_fasttimer.py" >>cmd_ft.sh
  echo "${steps[1]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step2_gpu.resources.json');process.FastTimerService.enableDQMbyLumiSection = cms.untracked.bool(False);process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step2_gpu_fasttimer.py" >>cmd_ft.sh
  echo "${steps[2]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step3_gpu.resources.json');process.FastTimerService.enableDQMbyLumiSection = cms.untracked.bool(False);process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step3_gpu_fasttimer.py" >>cmd_ft.sh
