...
if ( echo $outname | grep -q '136' ); then
  foor ((step=0;step<${#steps[@]}; ++step));do
...
# For reHLT workflows the steps are shifted
if ( echo $outname | grep -q '136') ; then
  echo "${steps[0]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step2_gpu_L1REPACK_HLT.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step2_gpu_fasttimer.py" >>cmd_ft.sh
  echo "${steps[1]}--accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step3_gpu_RAW2DIGI_L1Reco_RECO_SKIM_PAT_ALCA_DQM.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step3_gpu_fasttimer.py" >>cmd_ft.sh
  echo "${steps[2]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step4_gpu_HARVESTING.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step4_gpu_fasttimer.py" >>cmd_ft.sh
else
  echo "${steps[0]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step1_gpu_GEN_SIM.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step1_gpu_fasttimer.py" >>cmd_ft.sh
  echo "${steps[1]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step2_gpu_DIGI_L1TrackTrigger_L1_DIGI2RAW_HLT_PU.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step2_gpu_fasttimer.py" >>cmd_ft.sh
  echo "${steps[2]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step3_gpu_RAW2DIGI_L1Reco_RECO_RECOSIM_PU.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step3_gpu_fasttimer.py" >>cmd_ft.sh
  echo "${steps[3]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step4_gpu_PAT_PU.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;numberOfThreads = 2\" --python_filename=step4_gpu_fasttimer.py" >>cmd_ft.sh
# check for 5th step
if ( echo ${!steps[@]} | grep -q 4 );then
  echo "${steps[4]} --accelerators gpu-nvidia --procModifiers pixelNtupletFit,gpu --customise=HLTrigger/Timer/FastTimer.customise_timer_service_singlejob --customise_commands \"process.FastTimerService.writeJSONSummary = cms.untracked.bool(True);process.FastTimerService.jsonFileName = cms.untracked.string('step5_gpu_NANO_PU.resources.json');process.options.numberOfConcurrentLuminosityBlocks = 1;\" --python_filename=step5_gpu_fasttimer.py" >>cmd_ft.sh
  fi
fi
. cmd_ft.sh
. cmd_ig.sh
