...
if [ -d /cvmfs/patatrack.cern.ch/externals/x86_64/rhel8/nvidia/cuda-11.8.0/bin ];then
  ...
  if [ "X$RUNALLSTEPS" != "X" ]; then
    if [ -f step1_gpu_nvprof.py ];then
        ...
        nsys profile --kill=sigkill --output=step1_gpu_nsys --export=sqlite --stats=true --trace=cuda,nvtx,osrt,openmp,mpi,oshmem,ucx --mpi-impl=openmpi --show-output=true cmsRun step1_gpu_nvprof.py -j step1_gpu_nsys_JobReport.xml >& step1_gpu_nsys.log
    ...
    if [ -f step2_gpu_nvprof.py ];then
        ...
        nsys profile --kill=sigkill --output=step2_gpu_nsys --export=sqlite --stats=true --trace=cuda,nvtx,osrt,openmp,mpi,oshmem,ucx --mpi-impl=openmpi --show-output=true cmsRun step2_gpu_nvprof.py -j step2_gpu_nsys_JobReport.xml >& step2_gpu_nsys.log
  ...
  if [ -f step3_gpu_nvprof.py ];then
      ...
      nsys profile --kill=sigkill --output=step3_gpu_nsys --export=sqlite --stats=true --trace=cuda,nvtx,osrt,openmp,mpi,oshmem,ucx --mpi-impl=openmpi --show-output=true cmsRun step3_gpu_nvprof.py  -j step3_gpu_nsys_JobReport.xml >& step3_gpu_nsys.log
  ...
  if [ -f step4_gpu_nvprof.py ];then
      ...
      nsys profile --kill=sigkill --output=step4_gpu_nsys --export=sqlite --stats=true --trace=cuda,nvtx,osrt,openmp,mpi,oshmem,ucx --mpi-impl=openmpi --show-output=true cmsRun step4_gpu_nvprof.py -j step4_gpu_nsys_JobReport.xml >& step4_gpu_nsys.log
  ...
  if [ -f step5_gpu_nvprof.py ]; then
      ...
      nsys profile --kill=sigkill --output=step5_gpu_nsys --export=sqlite --stats=true --trace=cuda,nvtx,osrt,openmp,mpi,oshmem,ucx --mpi-impl=openmpi --show-output=true cmsRun step5_gpu_nvprof.py -j step5_gpu_nsys_JobReport.xml >& step5_gpu_nsys.log
