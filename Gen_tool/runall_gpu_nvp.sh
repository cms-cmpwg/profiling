...
if [ -d /cvmfs/patatrack.cern.ch/externals/x86_64/rhel8/nvidia/cuda-11.8.0/bin ];then
  ...
  if [ "X$RUNALLSTEPS" != "X" ]; then
    if [ -f step1_gpu_nvprof.py ];then
        ...
        nsys stats  step1_gpu_nsys.nsys-rep > step1_gpu_nsys.txt
    else
    ...
    if [ -f step2_gpu_nvprof.py ];then
        ...
        nsys stats  step2_gpu_nsys.nsys-rep > step2_gpu_nsys.txt
    else
  ...
  if [ -f step3_gpu_nvprof.py ];then
      ...
      nsys stats step3_gpu_nsys.nsys-rep > step3_gpu_nsys.txt
  else
  ...
  if [ -f step4_gpu_nvprof.py ];then
      ...
      nsys stats step4_gpu_nsys.nsys-rep > step4_gpu_nsys.txt
  else
  ...
  if [ -f step5_gpu_nvprof.py ]; then
      ...
      nsys stats step5_gpu_nsys.nsys-rep > step5_gpu_nsys.txt
  else
