...
re_selftime = re.compile(
    r"(\[\d+\])\s*((\d+['])?\d+\.?\d+)\s*((\d+['])?\d+\.?\d+)\s*((\d+['])?\d+\.?\d+)\s/\s((\d+['])?\d+\.?\d+)\s*(.*)"
)
m = re_selftime.match(
    "[19]        9.8     254.59       0.12 / 254.47     TrackstersProducer::produce(edm::Event&, edm::EventSetup const&)"
)
# print(displaymatch(m))
assert m
re_parentfunc = re.compile(
    r"(\s*)((\d+['])?\d+\.?\d+)\s*(.........)\s*((\d+['])?\d+\.?\d+)\s/\s((\d+['])?\d+\.?\d+)\s*(.*)\s*(\[\d+\])"
)
n = re_parentfunc.match(
    "            4.9  .........     127.31 / 2'361.48     edm::stream::EDProducerAdaptorBase::doEvent(edm::EventPrincipal const&, edm::EventSetupImpl const&, edm::ActivityRegistry*, edm::ModuleCallingContext const*) [15]"
)
# print(displaymatch(n))
assert n
o = re_selftime.match(
    "[1]       100.0   2'597.60       0.00 / 2'597.60   <spontaneous>\n"
)
...
global_old_df["SelfTotal"] = (
    global_old_df["SelfTotal"].str.replace("'", "").astype("float")
)
global_new_df["SelfTotal"] = (
    global_new_df["SelfTotal"].str.replace("'", "").astype("float")
)

global_old_df["SelfCumulative"] = (
    global_old_df["SelfCumulative"].str.replace("'", "").astype("float")
)
global_new_df["SelfCumulative"] = (
    global_new_df["SelfCumulative"].str.replace("'", "").astype("float")
)

global_old_df["ChildrenCumulative"] = (
    global_old_df["ChildrenCumulative"].str.replace(",", "").astype("float")
)
global_new_df["ChildrenCumulative"] = (
    global_new_df["ChildrenCumulative"].str.replace(",", "").astype("float")
)

global_old_df = global_old_df.sort_values(
    by=["PercentageTotal"], axis=0, ascending=False
)
global_new_df = global_new_df.sort_values(
    by=["PercentageTotal"], axis=0, ascending=False
)
...
for Tot_str in Tot_strs:
    ...
    print(
        "[{0:<4} -> {1:<4}] [{2:<6} -> {3:<6}]  [{4:<9} -> {5:<9}]         [ {6:<9}/{7:<9} - {8:<9}/{9:<9} *100% ] = [{10:<6}% ]         {11:<50}".format(
            idx_old[0],
            idx_new[0],
            global_old_df.loc[idx_old[0]]["PercentageTotal"],
            global_new_df.loc[idx_new[0]]["PercentageTotal"],
            global_old_df.loc[idx_old[0]]["SelfTotal"],
            global_new_df.loc[idx_new[0]]["SelfTotal"],
            newVal,
            newAll,
            oldVal,
            oldAll,
