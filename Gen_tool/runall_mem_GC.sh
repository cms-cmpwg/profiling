...
if [ "X$RUNALLSTEPS" != "X" ]; then
  if [ -f step1_igprof.py ]; then
    ...
    timeout $TIMEOUT igprof -mp -z -d -o ./igprofMEM_GC_step1.gz -- cmsRunGlibC step1_igprof.py -j step1_igprof_mem_GC_JobReport.xml >& step1_igprof_mem_GC.log
  ...
  if [ -f step2_igprof.py ]; then
    ...
    timeout $TIMEOUT igprof -mp -z -d -o ./igprofMEM_GC_step2.gz -- cmsRunGlibC step2_igprof.py -j step2_igprof_mem_GC_JobReport.xml >& step2_igprof_mem_GC.log
    rename_igprof igprofMEM_GC_step1
  else
...
if [ -f step3_igprof.py ]; then
    ...
    timeout $TIMEOUT igprof -mp -z -d -o ./igprofMEM_GC_step3.gz -- cmsRunGlibC step3_igprof.py -j step3_igprof_mem_GC_JobReport.xml >& step3_igprof_mem_GC.log
...
if [ -f step4_igprof.py ]; then
    ...
    timeout $TIMEOUT igprof -mp -z -d -o ./igprofMEM_GC_step4.gz -- cmsRunGlibC step4_igprof.py -j step4_igprof_mem_GC_JobReport.xml >& step4_igprof_mem_GC.log
...
if [ $(ls -d step5*.py | wc -l) -gt 0 ]; then
    ...
    timeout $TIMEOUT igprof -mp -z -d -o ./igprofMEM_GC_step5.gz -- cmsRunGlibC step5_igprof.py -j step5_igprof_mem_GC_JobReport.xml >& step5_igprof_mem_GC.log
