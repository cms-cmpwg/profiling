...
CMSRUN=`which cmsRun`
VTUNE=`which vtune`
$VTUNE -collect hotspots -data-limit=0 -knob enable-stack-collection=true -knob stack-size=4096 -knob sampling-mode=sw -- $CMSRUN $(ls TTbar*.py) >step1.log
$VTUNE -collect hotspots -data-limit=0 -knob enable-stack-collection=true -knob stack-size=4096 -knob sampling-mode=sw -- $CMSRUN $(ls step2*.py) >step2.log
$VTUNE -collect hotspots -data-limit=0 -knob enable-stack-collection=true -knob stack-size=4096 -knob sampling-mode=sw -- $CMSRUN $(ls step3*.py) >step3.log
$VTUNE -collect hotspots -data-limit=0 -knob enable-stack-collection=true -knob stack-size=4096 -knob sampling-mode=sw -- $CMSRUN $(ls step4*.py) >step4.log
