...
if [ "X$RUNALLSTEPS" != "X" ]; then
  if [ -f step1_igprof.py ]; then
    ...
    timeout $TIMEOUT igprof -mp -z -d -t cmsRun -o ./igprofMEM_step1.gz -- cmsRun step1_igprof.py -j step1_igprof_mem_JobReport.xml >& step1_igprof_mem.log
  ...
  if [ -f step2_igprof.py ]; then
    ...
    timeout $TIMEOUT igprof -mp -z -d -t cmsRun  -o ./igprofMEM_step2.gz -- cmsRun step2_igprof.py -j step2_igprof_mem_JobReport.xml >& step2_igprof_mem.log
...
if [ -f step3_igprof.py ]; then
    ...
    timeout $TIMEOUT igprof -mp -z -d -t cmsRun  -o ./igprofMEM_step3.gz -- cmsRun step3_igprof.py -j step3_igprof_mem_JobReport.xml >& step3_igprof_mem.log
...
if [ -f step4_igprof.py ]; then
    ...
    timeout $TIMEOUT igprof -mp -z -d -t cmsRun  -o ./igprofMEM_step4.gz -- cmsRun step4_igprof.py -j step4_igprof_mem_JobReport.xml >& step4_igprof_mem.log
...
if [ $(ls -d step5*.py | wc -l) -gt 0 ]; then
    ...
    timeout $TIMEOUT igprof -mp -z -d -t cmsRun  -o ./igprofMEM_step5.gz -- cmsRun step5_igprof.py -j step5_igprof_mem_JobReport.xml >& step5_igprof_mem.log
